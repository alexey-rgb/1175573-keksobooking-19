...
(function () {
  ...
  // готовит к рендеру карточку описания недвижимости
  ...
  var renderCard = function (item) {
    ...
    cardElement
      ...
      .appendChild(renderFeatures(item.offer.features));
    cardElement.querySelector(".popup__description").textContent =
      item.offer.description;
    cardElement
      .querySelector(".popup__photos")
      .appendChild(renderPhotos(item.offer.photos));
    cardElement.querySelector(".popup__photo").remove();
    cardElement.querySelector(".popup__close").setAttribute("tabindex", "1");
    cardElement
      .querySelector(".popup__close")
      .addEventListener("click", controlCardVisibility);
    return cardElement;
  };

  // добавляет возможность закрыть с клавиатуры карточку описания объявления
